...
public class ArrCharOps {
    ...
    public static void main(String[] args) {
        ...
        // System.out.println(hashCode(arr1[1]));
        //System.out.println(hashCode(arr2[1]));
    }
    ...
    public static char charAt(char[] arr, int index) {
        char c = arr[index];
        for (int findex = index; index < arr.length; findex++) {
            c = arr[findex];
    ...
    public static int indexOf(char[] arr, char ch, int fromIndex) {
        if (arr.length == 0 || fromIndex >= arr.length) {
            return -1;
        }
    ...
    public static char[] concat(char[] arr1, char[] arr2) {
        ...
        char[] container = new char[arr1.length + arr2.length];
        for (int i = 0; i < arr1.length; i++) {
            container[i] = arr1[i];
        }
        for (int j = 0; j < arr2.length; j++) {
            container[arr1.length + j] = arr2[j];
        }
        return container;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        ...
        char[] container = new char[endIndex - beginIndex];
        for (int i = beginIndex; i < endIndex; i++) {
            container[i - beginIndex] = arr[i];
        }
        return container;
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        int n = arr.length;
        for (int i = 0; i < n; i++) {
            sum = sum + ((long) arr[i] * (long) Math.pow(7, n - 1 - i));
